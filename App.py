...
if uploaded_file is not None:
    try:
        ...
        tab_audit, tab_clean, tab_dashboard, tab_custom, tab_story = st.tabs([
            "🔍 Data Audit", "✨ Auto-Clean", "📈 Auto Dashboard", "🎨 Custom Chart Builder", "📝 AI Business Story"
        ])
        ...
        with tab_story:
            ...
            if len(numeric_cols) > 0:
                ...
                st.write(
                    ...
                    f"to * me *{active_df[top_col].max():,.2f}**, averaging **{active_df[top_col].mean():,.2f}**."
                )
                if len(categorical_cols) > 0:
